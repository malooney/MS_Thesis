...
i <-1
temp <- list()
temp_main <- data.frame(UPC=1:length(main_beer_drug_and_groc$SY))

for(i in 1:length(temp_main$UPC)){
  
temp$SY <- str_pad(main_beer_drug_and_groc[i,3], 2, "left", pad="0")
temp$GE <- str_pad(main_beer_drug_and_groc[i,4], 2, "left", pad="0")
temp$VEND <- str_pad(main_beer_drug_and_groc[i,5], 5, "left", pad="0")
temp$ITEM <- str_pad(main_beer_drug_and_groc[i,6], 5, "left", pad="0")
